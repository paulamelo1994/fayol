		...
		<form name="horario" method="post" enctype="multipart/form-data" action="">
		 <table align="center" border="0" cellspacing="2" width="100%"> 
			<tr>
				...
				$consulta = "SELECT * from horario_salas where id_sala = '$id_sala' and fecha >= '".$inicio_semana."' and fecha <= '".$fin_semana."' order by hora, fecha ";
				...
				while($obj = pg_fetch_object($rs)){
					if($aux==0){
						...
						if($obj->estado == 'No Disponible'){					
							//busco reserva CAMBIA PARA SALAS DE AQUI EN ADELANTE
							...
							if($obj->hora == $obj1->hora_inicio){ ?>
								<td class="pretty" id="pretty" width="80" bgcolor="<?=$obj->color?>" align="center" valign="middle" title="<? echo makeDate($obj->fecha)." <br> Docente: ".$objprof->nombre." <br> Asignatura: ".$obj1->asignatura."<br> Grupo: ".$obj1->grupo." <br><br> Hora Inicio: ".substr($obj1->hora_inicio,0,-3)." <br> Hora Finalizacion: ".substr($obj1->hora_final,0,-3); ?>">
									<? if($opcion==3){?><a href="/Comunidad/Informatica/AsignacionSalas/Formularios/cancelar.php?id=<? echo $obj1->id_body; ?>&opc=2">
							...
							<? if($opcion==2){?><a href="/Comunidad/Informatica/AsignacionSalas/Formularios/reservar.php?fecha_sel=<? echo $obj->fecha; ?>&hora=<? echo substr($obj->hora,0,-3); ?>&sala=<? echo $id_sala; ?>">.</a> <? }?></td><?
						}
