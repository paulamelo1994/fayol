...
foreach($events as $event){
	...
	$thejson[] = array("title"=>$event->nom_asignatura." - Salón ".$salon->num_aula.", Edificio ".$salon->num_edificio." - Docente: ".$event->nom_docente,"start"=>$event->date_ini."T".$event->time_ini,"end"=>$event->date_ini."T".$event->time_fin, "backgroundColor"=>$color);
	
	$intermedias =fechasIntermedias($event->date_ini,$event->date_fin,'7');
	foreach($intermedias as $res){
		$thejson[] = array("title"=>$event->nom_asignatura." - Salón ".$salon->num_aula.", Edificio ".$salon->num_edificio." - Docente	: ".$event->nom_docente,"start"=>$res."T".$event->time_ini,"end"=>$res."T".$event->time_fin, "backgroundColor"=>$color);
	}
