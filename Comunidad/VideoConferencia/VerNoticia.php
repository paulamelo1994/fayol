<?
  ...
  {
  	$id = $_GET[id];
  	$tema = pg_escape_string( $_POST['nombre_evento'] );
	$solicitante = pg_escape_string( $_POST['organizador'] );
    $fecha = pg_escape_string( $_POST['fecha'] );
    $hora_inicio = pg_escape_string( $_POST['hora_inicio'] );
    $duracion = pg_escape_string( ($_POST['hora_inicio'] + 3).':00' );
    $tipo_evento = pg_escape_string( $_POST['tipo_evento'] );
	$protocolo = pg_escape_string( $_POST['descripcion'] );
	$correo_sol = pg_escape_string( $_POST['email'] );
	$rs = @db_query("update video_conferencia set nombre_evento = '$nombre_evento',
  ...
  $rs = pg_exec("SELECT * FROM video_conferencia WHERE id=$_GET[id]");
  ...
  echo Titulo("Descripcion del Evento<BR>&quot;$obj->tema&quot;")."<BR>";
  ...
  else
  {
		...
		"<TABLE BORDER=0 WIDTH='80%'>
		 ...
		 if ($obj->tipo_evento == 'na') echo 'Nacional';  
		 if ($obj->tipo_evento == 'in') echo 'Internacional';  

		 
		 echo "</TD></TR>
		 <TR><TD BGCOLOR='#F0F0F0' WIDTH=150><B>Organiza:</B></TD><TD>$obj->solicitante</TD></TR>
		 <TR><TD BGCOLOR='#F0F0F0'><B>Fecha:</B></TD><TD>$obj->fecha</TD></TR>
		 <TR><TD BGCOLOR='#F0F0F0'><B>Hora Inicio:</B></TD><TD>$obj->hora_inicio</TD></TR>
		 <TR><TD BGCOLOR='#F0F0F0'><B>Hora Fin:</B></TD><TD>$obj->hora_inicio + $obj->duracion</TD></TR>
		 <TR><TD BGCOLOR='#F0F0F0' VALIGN='TOP'><B>Protocolo:</B></TD><TD>$obj->descripcion </TD></TR>
