<?
   ...
   PageInit('Actas del Comit&eacute; del programa de Contadur&iacute;a P&uacute;blica', '../../menu.php', 'left', 'top');
?>

<H1 class="shiny"><?= IMAGEN_ACTAS ?> Actas del Comit&eacute; del programa de Contadur&iacute;a P&uacute;blica</H1>
	<?
	if($_GET['agno']>2004){
		$anio=$_GET['agno'];	
		$query1 = db_query("select * from actas_facultad where fecha >='$anio-01-01' and fecha<= '$anio-12-31' and  tipo_acta='3' order by n_acta ASC");
...
<!--		<a href="index.php">Volver</a>
	...
	<?
		...
		{		
			$query1 = db_query("select distinct extract (YEAR from fecha)as anno from actas_facultad where tipo_acta='3' order by anno desc");
			if(pg_num_rows($query1) != 0 )
			{		   
				 while($obj1=pg_fetch_object($query1))
				 {?>
