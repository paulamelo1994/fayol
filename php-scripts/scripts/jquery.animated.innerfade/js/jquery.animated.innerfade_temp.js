	...
	this.each(function(){ 	
		...
		if (settings.controlBox != 'none')
		{
			if (settings.controlBoxClass != 'none'){
                            $(this).append("<div class='"+settings.controlBoxClass+"_left control-panel'>\n\
                                            <a class='back-button' href='#'>\n\ \n\
                                                <img id='buttonPrev' src='"+settings.controlButtonsPath+"/prev.png' alt='previous' style='border: none;' />\n\
                                            </a>\n\ </div>"
                                            //+"<a class='pause-button' href='#'><img src='"+settings.controlButtonsPath+"/pause.gif' alt='pause' style='border: none;' /></a> \n\ "
                                            //+"<a class='next-button' href='#'>\n\ <img src='"+settings.controlButtonsPath+"/next.gif' alt='next' style='border: none;' /></a></div>\n\ "
                                            +"<div class='"+settings.controlBoxClass+"_right control-panel'>\n\
                                            <a class='next-button' href='#'>\n\ \n\
                                                <img id='buttonNext' src='"+settings.controlButtonsPath+"/sgte.png' alt='next' style='border: none;' />\n\
                                             </a>\n\ </div>");
                        }else{ 
                            $(this).append("<div class='control-panel'>\n\
                                            <a class='back-button' href='#'><img src='"+settings.controlButtonsPath+"/previous.gif' alt='previous' style='border: none;' /></a> \n\ "
                                            //+"<a class='pause-button' href='#'><img src='"+settings.controlButtonsPath+"/pause.gif' alt='pause' style='border: none;' /></a> \n\ "
                                            //+"<a class='next-button' href='#'><img src='"+settings.controlButtonsPath+"/next.gif' alt='next' style='border: none;' /></a></div>");
                                            +"<div class='control-panel'><img src='"+settings.controlButtonsPath+"/next.gif' alt='next' style='border: none;' /></div>");
                        }
		...
		if (elements.length > 1) {
			...
			$(this).css('position', 'relative').css('overflow', 'hidden').css('height', settings.containerheight).css('width', 740/*settings.containerwidth*/);
...
{	
	...
	if (settings.controlBox != 'none')
	{
		$(this).find(".control-panel a.next-button").unbind('click');
                $(".control-panel a.next-button").bind('click', function(){pauseActivated = false;clearTimeout(mytimer);
                //$(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.gif' alt='pause' style='border: none;' />"); 
                $.animatedinnerfade.next(elements, settings, next, current, mytimer, pauseActivated);return false;});
            
		$(this).find(".control-panel a.back-button").unbind('click'); 
                $(".control-panel a.back-button").bind('click', function(){pauseActivated = false; clearTimeout(mytimer);
                //$(".control-panel a.pause-button").html("<img src='"+settings.controlButtonsPath+"/pause.gif' alt='pause' style='border: none;' />"); 
                $.animatedinnerfade.next(elements, settings, prev, current, mytimer, pauseActivated);return false;});
		$(this).find(".control-panel a.pause-button").unbind('click');
                
                
                $(".control-panel a.pause-button").bind('click', function(){
                            clearTimeout(mytimer);
                            if (!pauseActivated){
                                    pauseActivated = true;
