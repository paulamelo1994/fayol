...
-->
		...
		<tr>
			...
			if($_SESSION['profesor']['permisos'] == 'total')
			{
				...
				$rs4 = db_query("SELECT distinct on (codigosala) codigosala from computador;");
			...
			else
			{
				for($i = 1; $i <= 2; $i++)
				{
					...
					if($i != $id_sala)
					{
						...
						<tr>
							<td align="center">
							<input type="radio" name="sala" value="<?=$i?>" title="Sala No. <?=$i?>" onClick="document.location.href='disponibilidad.php?sala=<?=$i?>'">
							<br>Sala <?=$i?></td>
						</tr>
						<?
					}
					else
					{
						...
						<tr>
							<td align="center">
							<input type="radio" name="sala" value="<?=$i?>" title="Sala No. <?=$i?>" checked onClick="document.location.href='disponibilidad.php?sala=<?=$i?>'">
							<br>Sala <?=$i?></td>
						</tr>
						<?
					}
			...
			{
			?>
			<tr><td align="center" colspan="7"><h2>Sala No. <?=$id_sala?><br><?=makeMonth($fe|cha);?></h2></td></tr>
			<? 
			}
			else
			{			
				?><tr><td align="center" colspan="7"><h2>Auditorio No. <?=$id_sala?><br><?=makeMonth($fecha);?></h2></td></tr>
				<?
			}
			?>
			<tr>
			...
			{
				$rs1 = db_query("select distinct(estado),fecha from horario where id_sala = '$id_sala' and fecha = '$obj->fecha' and hora != '13 - 14' and hora != '17 - 18' and hora != '21 - 22'");
	...
	if(isset($_GET['semana']))
	{
		...
		<table width="80%" align="center" border="0" cellspacing="2">
			<tr>
				...
				<? }else
				{?>
				<td align="center" colspan="7" valign="top"><h2>Auditorio No. <?=$id_sala?></h2><h3><?=makeDate($fecha)?></h3><br>Semana del:<br><?=makeDate($inicio_semana);?><b> - </b><?=makeDate($fin_semana);?><br></td></tr>
				<? } ?>
			...
			while($obj = pg_fetch_object($rs))
			{
				...
				if($obj->estado == 'Disponible')
				{
					...
					{
						if($obj->fecha >= $fecha_hoy)
						{
							?>
							<td height="20" bgcolor="<?=$obj->color?>" align="center" valign="middle" title="<?=makeDate($obj->fecha)?>. <?=$obj->estado?>">
							<a href="<?=makeURL("solicitudReserva.php?fecha_sel=$obj->fecha&amp;id_sala=$id_sala&amp;hora=$obj->hora")?>">O 
							</a>
							</td>
							<?
						}
						...
						{
							?>
							<td height="20" bgcolor="<?=$obj->color?>" align="center" valign="middle" title="<?=makeDate($obj->fecha)?>. <?="No disponible"?>">
					...
					{
						?>
						<td height="20" bgcolor="<?=$obj->color?>" align="center" valign="middle" title = "<?=makeDate($obj->fecha)?>. <?="No disponible ww"?>"></td>
						<?
					}
				}
				else
