<?
	...
	*********************************************************/
	...
	if((!isset($_SESSION['adminSalas'])) && ($opcion == 1)){
		...
		$consulta = "SELECT distinct on (id_sala, fecha) * from horario_salas where id_sala = '$id_sala' and fecha like '".splitDate($fecha, 'year')."-".splitDate($fecha, 'month')."%' ;";
		...
		if(pg_num_rows($rs) == 0)
		{
			for($i = 1; $i <= $dias_mes; $i++)
			{
				...
				if($i < 10) $i = "0".$i;
				$tmp1 = splitDate($fecha, 'year')."-".splitDate($fecha, 'month')."-".$i;
				$tmp=pg_escape_string($tmp1);
				for($j = 7; $j < 22; $j++)
				{
					if($j < 10){
						$h="0".$j.":00";
					...
					$h11=pg_escape_string($h);
					
					$consulta1 = "insert into horario_salas (id_sala, fecha, hora, estado, color) values('$id_sala', '$tmp', '$h11', 'Disponible', '#FFFFFF')";
        ...
        <table width="70%" border="0" align="center" cellpadding="1">
			...
			<tr bgcolor="#FFFFFF">
				...
				{
					$rs1 = db_query("select count(*) as cantidad from horario_salas where id_sala = '$id_sala' and fecha = '$obj->fecha' and estado='Disponible'");
					...
					if($obj1->cantidad==15)
					{
						...
						<td bgcolor="#FFFFFF" height="50" align="center" valign="middle" title="Disponible" style="border:1px; border-color:#AAAAAA; ">
									...
									<a href="calendReserva.php?sala=<? echo $id_sala;?>&opcion=<? echo $opcion; ?>&fecha=<?=$obj->fecha?>" class="dayOn">
					...
					if($obj1->cantidad==0)
					{
						...
						<td bgcolor="#FFFFFF" height="50" align="center" valign="middle" class="dayOff" title="No Disponible"  style="border:1px; border-color:#AAAAAA; ">
									...
									<a href="calendReserva.php?sala=<? echo $id_sala;?>&opcion=<? echo $opcion;?>&fecha=<?=$obj->fecha?>" class="dayOn">
									<font color="#e04e4f"><?=splitDate($obj->fecha, 'day')?></font>
									</a>
						</td>
						<?
					}
					if(($obj1->cantidad > 0)&&($obj1->cantidad < 15))
					{
						...
						<td bgcolor="#FFFFFF" height="50" align="center" valign="middle" title="Parcialmente Disponible"  style=" border:1px; border-color:#AAAAAA; ">
							...
							<a href="calendReserva.php?sala=<? echo $id_sala;?>&opcion=<? echo $opcion;?>&fecha=<?=$obj->fecha?>" class="dayOn">
							<font color="#0099CC"><?=splitDate($obj->fecha, 'day')?></font>
							</a>
						</td>
						<?
					}
