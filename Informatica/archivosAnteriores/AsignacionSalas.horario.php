<?
	...
	Archivo: horario.php
	Objetivo: Este archivo muestra el horario de las salas, las horas libres y las reservadas. Inicialmente tiene
				modo mes pero al seleccionar uno de los dias te enseña la semana completa.
	...
	*********************************************************/
	...
	//echo "SELECT distinct on (id_sala, fecha) * from horario where id_sala = $id_sala and fecha like '".splitDate($fecha, 'year')."-".splitDate($fecha, 'month')."%'";
		...
		if(!isset($_SESSION['profesor'])){
		PageInit("Horario del Mes : Sala $id_sala", "../../menu.php");
		}else{
		PageInit("Horario del Mes : Sala $id_sala", "menu.php");
		}
		?>
		<h1 class="shiny">Horario</h1>
		...
		<tr>
			...
			<table width="100%" border="0">
			<tr>
				<td class="titulosContenidoInterno">A continuaci&oacute;n puede escoger la sala deseada:
			...
			<?
				...
				if($i != $id_sala)
				{
					...
					<tr>
						<td align="center">
						<input type="radio" name="sala" value="<?=$i?>" title="Sala No. <?=$i?>" onChange="document.location.href='horario.php?sala=<?=$i?>'"><br>Sala <?=$i?>
						</td>
					</tr>
					<?
				}
			...
			<td width="80%">		  <table width="70%" align="center" border="1" cellspacing="1">
              <tr>
                <td align="center" colspan="7"><h2>Sala No.
                        <?=$id_sala?>
                        <br>
                        <?=makeMonth($fecha);?>
                </h2></td>
              ...
              <tr>
                <td colspan="1" align="center" valign="middle"><input type="image" src="<?=$root_path?>/Images/atras.jpg" name="atras" title="Mes Anterior" value="<?=$mes_anterior?>">
                </td>
                <td colspan="5"></td>
                <td colspan="1" align="center" valign="middle"><input type="image" src="<?=$root_path?>/Images/adelante.png" name="siguiente" title="Mes Siguiente" value="<?=$mes_siguiente?>">
                </td>
              </tr>
              <tr>
              ...
              <tr bgcolor="#FFFFFF">
				...
				while($obj = pg_fetch_object($rs))
				{
					...
					{
						?>
                <td bgcolor="#FFFFFF" height="50" align="center" valign="middle" title = "Disponible"><font color="#000000" class="dayOn">
                  <?=splitDate($obj->fecha, 'day')?>
                </font></td>
                <?
					}
					...
					{
						?>
                <td bgcolor="#FFFFFF" height="50" align="center" valign="middle" class="dayOff" title = "No Disponible"><a href="horario.php?semana=true&amp;seleccion=<?=$obj->fecha?>&amp;sala=<?=$id_sala?>" class="dayOff">
                  <?=splitDate($obj->fecha, 'day')?>
                </a> </td>
                <?
					}
					...
					{
						?>
                <td bgcolor="#FFFFFF" height="50" align="center" valign="middle" title = "Parcialmente Disponible"><a href="horario.php?semana=true&amp;seleccion=<?=$obj->fecha?>&amp;sala=<?=$id_sala?>" class="dayOn"><font color="#0099CC">
                  <?=splitDate($obj->fecha, 'day')?>
                </font></a> </td>
                <?
					}
	...
	if(isset($_GET['semana']))
	{
		...
		else $fecha = date('Y')."-".date('m')."-".date('d');
		...
		<table align="center" border="0" cellspacing="2" width="100%">
			<tr>
				...
				<table align="center" border="0" cellspacing="2" width="15%">
				<tr><td><b>Sala:</b></td></tr>
				<?
				DBConnect('controlsalas');
				
				$rs5 = db_query("SELECT distinct on (codigosala) codigosala from computador where disponible='true';");
				
				while($obj5 = pg_fetch_object($rs5))
				{
					?>
					<tr><td><br></td></tr>
					<?
					...
					if($i != $id_sala)
					{
						...
						<tr>
							<td align="center">
							<input type="radio" name="sala" value="<?=$i?>" title="Sala No. <?=$i?>" onClick="document.location.href='horario.php?semana=true&amp;seleccion=<?=$fecha?>&amp;sala=<?=$i?>'">
							<br>Sala <?=$i?></td>
						</tr>
						<?
					}
					else
					{
						...
						<tr>
							<td align="center">
							<input type="radio" name="sala" value="<?=$i?>" title="Sala No. <?=$i?>" checked disabled onClick="document.location.href='horario.php?semana=true&amp;seleccion=<?=$fecha?>&amp;sala=<?=$i?>'">
							<br>Sala <?=$i?></td>
						</tr>
						<?
					}
				...
				<tr>
					<td colspan="1" valign="middle" align="center"><a href="horario.php?sala=<?=$id_sala?>">
					<img border="1" src="<?=$root_path?>/Images/iconoMes.png" name="volver_mes" title="Volver a modo mes!" width="30" height="30" alt=""></a>
					</td>
					<td align="center" colspan="6" valign="top"><h2>Sala No. <?=$id_sala?></h2><br>Semana del:<br><?=makeDate($inicio_semana);?><b> - </b><?=makeDate($fin_semana);?><br></td>
					<td colspan="1" align="center" valign="middle"><a href="horarioPDF.php?semana=<?=$fecha?>" target="_blank" title="Generar PDF!">
					<img src="../../../Images/PDFMiniFile.jpg" title="Generar PDF!" alt=""></a>
					</td>
				</tr>
				<tr>
					<td colspan="1" align="center" valign="middle">
					<a href="horario.php?semana=true&amp;seleccion=<?=$semana_anterior?>&amp;sala=<?=$id_sala?>">
					<img src="<?=$root_path?>/Images/atras.jpg" name="semana_atras" title="Semana Anterior"  alt=""></a>
					</td>
					<td colspan="6"></td>
					<td colspan="1" align="center" valign="middle">
					<a href="horario.php?semana=true&amp;seleccion=<?=$semana_siguiente?>&amp;sala=<?=$id_sala?>">
					<img src="<?=$root_path?>/Images/adelante.png" name="semana_siguiente" title="Semana Siguiente" alt=""></a>
					</td>
				</tr>
					...
					{					
						...
						{					
							...
							if($obj->hora == '07 - 08' || $obj->hora == '10 - 11' || $obj->hora == '14 - 15' || $obj->hora == '18 - 19')
							{
								?>
								<td height="20" bgcolor="<?=$obj->color?>" align="center" valign="middle" title="<?=makeDate($obj->fecha)?>. <?=$obj->estado?>">
								<a href="<?=makeURL("editar.php?indice=$obj1->indice")?>"><b><font color="#FFFFFF"><?=$obj1->asignatura?></font></b></a>
								</td>
								<?
							}
							else
							{
								...
								if($obj->hora == '09 - 10' || $obj->hora == '12 - 13' || $obj->hora == '16 - 17' || $obj->hora == '20 - 21')
								{
									...
									else
									{
										?>
										<td height="20" bgcolor="<?=$obj->color?>" align="center" valign="middle" title="<?=makeDate($obj->fecha)?>. <?=$obj->estado?>">
										<font color="#FFFFFF"><?=$obj1->software?></font>
										</td>
										<?
									}
								}
