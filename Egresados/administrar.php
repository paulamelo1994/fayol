  ...
  PageInit("Egresados", "../menu.php", 8);
	...
	if(isset($_POST['entrar']))
	{
		...
		if(!empty($conexion)) //Si hay conexion
		{
			...
			if ( $numrows == 1 )
				$_SESSION['egresados'] = true;
			else
				echo "<strong>Aceeso no permitido.</strong>";
		}
	}

	if(isset($_SESSION['egresados']))
	{
		if(isset($_FILES['archivo']))
		{
			...
			if(move_uploaded_file($_FILES['archivo']['tmp_name'], $target_path)) 
			{
				...
				DBConnect('new_fayol');
				db_query("insert into archivo(nombre, descripcion, direccion, id_categoria, visible ) values('$_POST[titulo]', '$_POST[titulo]', '$target_path', 2, true)");
		...
		{
				$item = $_GET['item'];
				$esconder = $_GET['esconder'];
				$id = $_GET['id'];
				
				//print("entra ".$item."  ".$esconder." id ".$id );
			...
			<div align="right">
			    <p><a href="archivos.php?item=5">Cerrar sesi&iacute;on</a></p>
			...
			<FORM METHOD="POST" enctype="multipart/form-data" action="">
			  <table width="90%"  border="0">
				...
				<tr>
				  ...
				  <td width="44%"><input type="hidden" name="MAX_FILE_SIZE" value="100000000000000">
			...
			<?
				...
				{
					$res = db_query("SELECT * FROM archivo where id_categoria = 2 order by fecha desc;");
					$numrows = pg_num_rows($res);
